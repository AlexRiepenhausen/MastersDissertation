...
if __name__ == '__main__':
    ...
    train_labels = p.imdb_lbl_pos_train   + p.imdb_lbl_neg_train
    test_files   = p.imdb_files_pos_test  + p.imdb_files_neg_test
    test_labels  = p.imdb_lbl_pos_test    + p.imdb_lbl_neg_test#
    ...
    if mode == Mode.word2vec:
        ...
        w2v = Word2VecTrainer(primary_files=train_files,
                              emb_dimension=100,
                              batch_size=32,
                              window_size=5,
                              initial_lr=0.01,
        ...
        parcel_0 = w2v.train(train_files, p.dict_file, num_epochs=10)
    ...
    if mode == Mode.conversion:
        ...
        utilities.documentVectorisation(train_files, p.vec_files_train, p.dict_file, unknown_vec=Vec.zeroVec)
        utilities.documentVectorisation(test_files, p.vec_files_test, p.dict_file, unknown_vec=Vec.zeroVec)
